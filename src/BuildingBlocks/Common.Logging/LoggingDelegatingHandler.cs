...
namespace Common.Logging
{
    public class LoggingDelegatingHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Sending request to {Uri} - {Method} - {Version}", 
                ...
                {
                    _logger.LogInformation("Received a success response from {Uri}", response.RequestMessage?.RequestUri);
                }
                else
                {
            ...
            when(ex.InnerException is SocketException { SocketErrorCode: SocketError.ConnectionRefused})
            {
                ...
                _logger.LogCritical(ex, "Unable to connect to {Host}. Please check the" +
                    "configuration/settings to ensure the correct URL for service" +
